...
NAME = ''  # e.g. onlyny | Has to be the same as on url (lowercase)
cats = []
        ...
        for d in data:
            ...
            for color in colors:
                ...
                for fit in fits:
                    ...
                    if len(prod.img_urls) == 0: continue
                    tot += 1
                    prods.append(prod)
                    rows += prod.get_rows()
        ind += 1
...
write_file(NAME.title()+'.txt', "Total products: " + str(tot) + "/" + str(sum(list(reason.values()))+tot), "Total Time: " + str(round(time()-start, 2)) + 's', reason)
