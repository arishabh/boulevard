...
for c in cats:
    ...
    while data:
        for d in data:
            ...
            if err: continue
            prod = Shopify(d, c, NAME, DISPLAY_NAME)  # Initialize Shopify product
            prod.tags = [gen_clean(d['vendor']),
                         gen_clean(c)]  # Set the products to be the vendor and the category by default
            colors = []
            all_sizes = []
            fits = []
            waist = []
            ...
            for option in d['options']:
                ...
                elif (option['name'].lower() == 'size' or option['name'] == 'Title' or option['name'] == 'US Size'):
