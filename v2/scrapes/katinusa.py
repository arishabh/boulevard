...
NAME = 'katinusa' #e.g. onlyny | Has to be the same as on url
DISPLAY_NAME = 'Katin'
...
def proc_size(size):
    ...
    if(size == 'O/S'): return 'OS'
    return size

for c in cats:
    ind = 1
    data = json.loads(bs(get(url[0]+c+url[1]+'1').content, 'html.parser').getText())['products']
    # For each catagory in given in above, get its json
    while(data != []):
        for d in data:
